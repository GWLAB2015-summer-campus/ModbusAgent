...
int main(int argc, char*argv[])
{
	...
	char sPortName[MAX_BUF];
	char sSlaveID[MAX_BUF];
	char sBitRate[MAX_BUF];
	char sParity[MAX_BUF];
	char sDebufOption[MAX_BUF];
	...
	if ( (fp = fopen("serv_option.txt", "rt")) == NULL )
	{
		printf("option file open error.\n");
		exit(1);
	}
	
	fscanf( fp, "%s", sPortName );
	fscanf( fp, "%s",sSlaveID );
	fscanf( fp, "%s",sBitRate );
	fscanf( fp, "%s",sParity );	
	fscanf( fp, "%s",sDebufOption );
	// read option in txt
	
	fclose(fp);

	nSlaveID = atoi(sSlaveID);
	nBitRate = atoi(sBitRate);
	// translate string to int

	ctx = modbus_new_rtu(sPortName, nBitRate, sParity[0], 8, 1);

	if ( !strcmp( sDebufOption, "ON" ))
		modbus_set_debug(ctx, TRUE);

	query = malloc(MODBUS_RTU_MAX_ADU_LENGTH);
	header_length = modbus_get_header_length(ctx);

	modbus_set_slave(ctx, nSlaveID);
